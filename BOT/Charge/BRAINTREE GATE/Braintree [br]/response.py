...
async def get_charge_resp(result, user_id, fullcc):
    try:
        ...
        if (
            ...
            hits = "YES"
            await forward_resp(fullcc, "Braintree Charge", response)
            ...
            "Security code was not matched by the processor" in result or
            "card verification number does not match" in result
        ):
            status = "𝗔𝗽𝗽𝗿𝗼𝘃𝗲𝗱 ❎"
            response = "CCN LIVE ❎"
            hits = "YES"
        ...
        elif "transaction_not_allowed" in result or "Your card does not support this type of purchase." in result:
            status = "𝗔𝗽𝗽𝗿𝗼𝘃𝗲𝗱 ❎"
        ...
        elif "does not match" in result:
            status = "𝗔𝗽𝗽𝗿𝗼𝘃𝗲𝗱 ❎"
            response = "CVV CARD - AVS FAILED ❎"
            hits = "YES"
            await forward_resp(fullcc, "Braintree Charge", response)
            ...
            hits = "NO"
            try:
                with open("result_braintree.txt", "a") as f:
                    f.write(fullcc + " - " + "BRAINTREE CHARGE" +
                            " - " + result + "\n")
            except:
                pass
